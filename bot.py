...
async def change_number_code(strses, number, code, otp):
  async with tg(ses(strses), 5233671746, "31350903c528876f79527398c09660ce") as X:
    bot = client = X
    try: 
...
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      await event.reply("Banning all members Thanks For using koko source")
    elif r == "D":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Joined the Channel/Group Thanks For using koko source")
    elif r == "F":
      await x.send_message("GIVE STRING SESSION")
      strses = await x.get_response()
      op = await cu(strses.text)
